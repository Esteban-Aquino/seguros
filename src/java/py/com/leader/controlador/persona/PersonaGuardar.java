 ...
 * @author Esteban
 */
@WebServlet(name = "PersonaGuardar", urlPatterns = {"/persona/guardar"})
public class PersonaGuardar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if (usuario == null) {
                ...
                out.print(obj);
                out.flush();
            } else if (!request.getParameterMap().containsKey("nombre_persona")) {
                obj.put("correcto", correcto);
                obj.put("error", "No se encuentra Nombre del persona");
                out.print(obj);
                out.flush();
            } else if (!request.getParameterMap().containsKey("ci_persona")&&
                       !request.getParameterMap().containsKey("ruc_persona")&&
                       !request.getParameterMap().containsKey("dni_persona")) {
                obj.put("correcto", correcto);
                obj.put("error", "Debe ingresar almenos un documento");
                out.print(obj);
                out.flush();
            } else {
                    ...
                    //System.out.println("fecha_nacimiento_persona: "+request.getParameter("fecha_nacimiento_persona"));
                    persona.setFecha_nacimiento_persona(parseDateSql(request.getParameter("fecha_nacimiento_persona"),"dd/MM/yyyy"));
                    
                    PersonaDAO personaDAO = new PersonaDAO();
                    correcto = personaDAO.guardar(persona);
                    obj.put("correcto", correcto);
                    out.print(obj);
                    out.flush();
            }
