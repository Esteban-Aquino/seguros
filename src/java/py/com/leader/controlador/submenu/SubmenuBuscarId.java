 ...
 * @author Esteban
 */
@WebServlet(name = "SubmenuBuscarId", urlPatterns = {"/submenu/buscar/id"})
public class SubmenuBuscarId extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                if (!request.getParameterMap().containsKey("id_submenu")) {
                    obj.put("correcto", correcto);
                    obj.put("error", "No se encuentra Id Submenu");
                    out.print(obj);
                    out.flush();
                } else {
                    SubmenuDAO submenuDAO = new SubmenuDAO();
                    Submenu submenu = submenuDAO.buscarId(Integer.parseInt(request.getParameter("id_submenu")));
                    out.print(submenu.getJSONObject());
                    out.flush();
                }
