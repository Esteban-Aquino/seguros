 ...
 * @author Esteban
 */
@WebServlet(name = "ClienteEliminar", urlPatterns = {"/cliente/eliminar"})
public class ClienteEliminar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                if (!request.getParameterMap().containsKey("id_cliente")) {
                    obj.put("correcto", correcto);
                    obj.put("error", "No se encuentra Id Cliente");
                    out.print(obj);
                    out.flush();
                } else {
                    ClienteDAO clienteDAO = new ClienteDAO();
                    correcto = clienteDAO.eliminar(Integer.parseInt(request.getParameter("id_cliente")));
                    obj.put("correcto", correcto);
                    out.print(obj);
                    out.flush();
                }
