 ...
 * @author Esteban
 */
@WebServlet(name = "UsuarioGuardar", urlPatterns = {"/usuario/guardar"})
public class UsuarioGuardar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                if (!request.getParameterMap().containsKey("usuario_usuario")) {
                    ...
                    persona.setId_persona(Integer.parseInt(request.getParameter("id_persona_usuario")));
                    usuario.setPersona(persona);
                    usuario.setNombre_usuario(request.getParameter("nombre_persona"));
                    usuario.setClave_usuario(request.getParameter("clave_usuario"));
                    String detalle = request.getParameter("detalle");
                    
                    UsuarioDAO usuarioDAO = new UsuarioDAO();
                    correcto = usuarioDAO.guardar(usuario,detalle);
                    obj.put("correcto", correcto);
                    out.print(obj);
                    out.flush();
                }
